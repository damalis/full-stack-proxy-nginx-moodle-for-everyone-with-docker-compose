...
```
git clone https://github.com/damalis/full-stack-proxy-nginx-moodle-for-everyone-with-docker-compose.git
```

Open a terminal and `cd` to the folder in which `docker-compose.yml` is saved and run:

```
cd full-stack-proxy-nginx-moodle-for-everyone-with-docker-compose
chmod +x install.sh
./install.sh
```
...
add and/or remove moodle site folders and files with any ftp client program in ```./moodle``` folder.
...
[Configuring](https://docs.moodle.org/401/en/Redis_cache_store#Configuring_Redis_in_Moodle)
